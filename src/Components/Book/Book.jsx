...
const[error,setError]=useState(false);
              ...
              else
              {
               setError(false);
                  const myobj={ 
                    ...
                    console.log(myobj)  
                    fetch("https://joyaljojo-69ea8-default-rtdb.firebaseio.com/book.json",
                    { method:"POST",body:JSON.stringify(myobj), headers:{"Content-Type":"application/json",}});
                    alert("Record Saved")}
}
 ...
 return (
  <div> Book Name<input type="text" onChange={readbookname} required/><br/> 
 ...
 Publisher<input type="text" onChange={readpub} required/><br/>
  <button type="submit" onClick={checkfilldata}>SAVE</button> 
   {error && 'All the fields must be entered'}</div>
    )
  }
export default Book